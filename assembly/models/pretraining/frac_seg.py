...
class FracSeg(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        with torch.no_grad():
            ...
            out_dict["valid_graph"] = valid_graph  # (B, P, P)
        ...
        points_offset = torch.cumsum(points_per_part[valid_pcs], dim=-1)  # (X,), points_per_part for valid parts

        with torch.autocast(device_type="cuda", dtype=torch.float16):
            super_point, point = self.encoder(
                {
                    "coord": part_pcds, # (B * N, 3)
                    "offset": points_offset, # (X,) X: number of valid parts
                    "feat": torch.cat([part_pcds, part_pcds], dim=-1),
                    "grid_size": torch.tensor(self.grid_size).to(part_pcds.device),
                }
            )
            points_features = point["feat"]  # (B * N, pc_feat_dim)
