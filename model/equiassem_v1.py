...
from model.backbone.vn_dgcnn import EQCNN_inv
...
class EquiAssem_v1(pl.LightningModule):
    def __init__(self, lr, visualize=False):
        ...
        # Feature Extractor
        self.backbone = EQCNN_inv(feat_dim=self.feat_dim, pooling="mean")
    ...
    def forward_pass(self, in_dict, mode, visualize=False):
        ...
        # 1. SO(3)-Invariant Feature Extractor
        src_inv_feats = self.backbone(src_pcd)
        trg_inv_feats = self.backbone(trg_pcd)
        ...
        # 3. Weighted SVD with top-k correspondence selections
        with torch.no_grad():
        ...
        # 4-2 point matching loss
        loss['p_loss'] = self.matching_loss(matching_scores, gt_corr, src_pcd_raw, trg_pcd_raw)

        # 4-3. final loss
        loss['loss'] = self.c_loss_weight * loss['c_loss'] + self.p_loss_weight * loss['p_loss']
