...
class EquiAssem_v1(pl.LightningModule):
    def __init__(self, lr, backbone='eqcnn', visualize=False):
        ...
        # Matcher
        self.matcher = nn.Sequential(nn.Conv1d(self.feat_dim//3*3*2, self.feat_dim, kernel_size=1),
    ...
    def forward_pass(self, in_dict, mode, optimizer_idx):
        ...
        out_dict['trg_ori'] = trg_ori # Save orientations for visualization

        # 5. Invariant Features
        src_inv_feats = torch.matmul(src_equi_feats.permute(0, 3, 1, 2), src_ori.transpose(-2,-1)) # (1, N, C//3*2, 3) x (1, N, 3, 3) -> (1, N, C//3*2, 3)
        trg_inv_feats = torch.matmul(trg_equi_feats.permute(0, 3, 1, 2), trg_ori.transpose(-2,-1)) # (1, M, C//3*2, 3) x (1, M, 3, 3) -> (1, M, C//3*2, 3)
        src_inv_feats = rearrange(src_inv_feats, 'b n c r -> b (c r) n') # (1, N, C//3*2, 3) -> (1, C*2, N)
        trg_inv_feats = rearrange(trg_inv_feats, 'b n c r -> b (c r) n') # (1, N, C//3*2, 3) -> (1, C*2, N)
