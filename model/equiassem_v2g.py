...
class EquiAssem_v2g(pl.LightningModule):
    def __init__(self, lr, visualize=False):
        super(EquiAssem_v2g, self).__init__()
        ...
        self.global_mlp = nn.Sequential(nn.Conv1d(self.feat_dim//3*3*2, self.feat_dim//3*3, kernel_size=1), 
    ...
    def forward_pass(self, in_dict, mode, visualize=False):
        ...
        if self.pooling == 'max':
            src_global_feats = torch.max(src_inv_feats, dim=-1, keepdim=True)[0].expand_as(src_inv_feats)
            trg_global_feats = torch.max(trg_inv_feats, dim=-1, keepdim=True)[0].expand_as(trg_inv_feats)
        elif self.pooling == 'mean':
            src_global_feats = torch.mean(src_inv_feats, dim=-1, keepdim=True).expand_as(src_inv_feats)
            trg_global_feats = torch.mean(trg_inv_feats, dim=-1, keepdim=True).expand_as(trg_inv_feats)
        
        src_inv_feats = torch.cat([src_inv_feats, src_global_feats], dim=1)
        src_inv_feats = self.global_mlp(src_inv_feats)
        trg_inv_feats = torch.cat([trg_inv_feats, trg_global_feats], dim=1)
        trg_inv_feats = self.global_mlp(trg_inv_feats)
        ...
        # 9-1. circle loss
        loss['c_loss'], loss['FMR'] = self.circle_loss(src_pcd_raw, trg_pcd_raw, src_inv_feats.transpose(-2,-1), trg_inv_feats.transpose(-2,-1), gt_corr)
    ...
    def evaluate_prediction(self, in_dict, out_dict, visualize=False, multi_part=False):
        ...
        #     save_pc(f"./vis/iou{round(iou,2)}_rrmse{round(eval_result['rrmse'].item(),1)}_crd{round(eval_result['crd'].item(),2)}_grtr.pcd", pcds_grtr)
