...
def knn(x, k):
    """
    Batch-wise kNN using torch.cdist
    x: (B, 3, N)
    return: idx (B, N, k)
    """
    B, _, N = x.shape
    ...
    # Batch-wise pairwise distance
    # dist: (B, N, N)
    dist = torch.cdist(x_t, x_t, p=2)
    
    # topk returns (values, indices)
    # _, idx = dist.topk(k=k, largest=False, dim=-1)
    idx = torch.topk(dist, k=k, largest=False, dim=-1).indices
    return idx
...
def get_graph_feature(x, k=20, idx=None, x_coord=None):
    ...
    feature = torch.cat((feature-x, x.expand(-1, -1, k, -1, -1)), dim=3).permute(0, 3, 4, 1, 2).contiguous()
...
def get_graph_feature_old(x, k=20, idx=None, x_coord=None):
    batch_size = x.size(0)
    num_points = x.size(3)
    x = x.view(batch_size, -1, num_points)
    if idx is None:
