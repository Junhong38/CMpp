...
class EquiAssem(pl.LightningModule):
    def __init__(self, lr, shape='local', occ='global', shape_loss='positive', occ_loss='negative', no_ori=False, visualize=False):
    ...
    def evaluate_prediction(self, in_dict, out_dict, gt_corr, multi_part=False):
        ...
        if self.visualize:
            ...
            save_pc(f"./vis_positive/crd{round(eval_result['crd'].item(),2)}_o_loss{round(out_dict['o_loss'].item(),2)}_occ_loss{round(out_dict['occ_loss'].item(),2)}_rrmse{round(eval_result['rrmse'].item(),1)}_pred.pcd", pcds_pred)
            save_pc(f"./vis_positive/crd{round(eval_result['crd'].item(),2)}_o_loss{round(out_dict['o_loss'].item(),2)}_occ_loss{round(out_dict['occ_loss'].item(),2)}_rrmse{round(eval_result['rrmse'].item(),1)}_grtr.pcd", pcds_grtr)
