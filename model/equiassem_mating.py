...
class EquiAssem(pl.LightningModule):
    def __init__(
            ...
            score_comb='intersection', topk=1, optimal_matching_choice='many-to-one'
            ):
        ...
        self.topk = topk
        self.optimal_matching_choice = optimal_matching_choice
    ...
    def forward_mpa(self, in_dict):
        ...
        with torch.no_grad():
            if self.registration == 'wsvd':
                if self.gt_mating_surface and torch.unique(in_dict['gt_correspondence'][0][:,0]).numel() > 3 and torch.unique(in_dict['gt_correspondence'][0][:,1]).numel() > 3:
                    _, M, N = matching_scores_drop.shape
                    gtms_src = torch.zeros(M, dtype=torch.bool)
                    gtms_trg = torch.zeros(N, dtype=torch.bool)
                    gtms_src[torch.tensor(list(set(in_dict['gt_correspondence'][0][:,0])))] = True
                    gtms_trg[torch.tensor(list(set(in_dict['gt_correspondence'][0][:,1])))] = True
                    if gtms_trg.sum() < 3:
                        breakpoint()
                    matching_scores_drop = matching_scores_drop[0][gtms_src][:, gtms_trg].unsqueeze(0)

                    src_pcd_for_ransac = src_pcd[gtms_src.unsqueeze(0)].unsqueeze(0)
                    trg_pcd_for_ransac = trg_pcd[gtms_trg.unsqueeze(0)].unsqueeze(0)

                else:
                    src_pcd_for_ransac = src_pcd
                    trg_pcd_for_ransac = trg_pcd

                # breakpoint()
                src_corr_pts, trg_corr_pts, corr_scores, estimated_transform, pred_corr = self.fine_matching(
                    src_pcd_for_ransac, trg_pcd_for_ransac, matching_scores_drop, k=128)

                # src_corr_pts, trg_corr_pts, corr_scores, estimated_transform, pred_corr = self.fine_matching(
                #     src_pcd, trg_pcd, matching_scores_drop, k=128)

                # R = estimated_transform[:3, :3]
                # t = estimated_transform[:3, 3]
                ...
                # matching_scores_before_Sinkhorn = matching_scores_drop

                # 9-1. Select initial matches
                matching_scores_before_Sinkhorn = matching_scores_before_Sinkhorn.squeeze(0)

                # if self.gt_mating_surface and in_dict['gt_mating_surface'][0].sum() > 3 and in_dict['gt_mating_surface'][1].sum() > 3:
                    # gtms_src = in_dict['gt_mating_surface'][0].squeeze(0)
                    # gtms_trg = in_dict['gt_mating_surface'][1].squeeze(0)
                if self.gt_mating_surface and torch.unique(in_dict['gt_correspondence'][0][:,0]).numel() > 3 and torch.unique(in_dict['gt_correspondence'][0][:,1]).numel() > 3:
                    M, N = matching_scores_before_Sinkhorn.shape
                    gtms_src = torch.zeros(M, dtype=torch.bool)
                    gtms_trg = torch.zeros(N, dtype=torch.bool)
                    gtms_src[torch.tensor(list(set(in_dict['gt_correspondence'][0][:,0])))] = True
                    gtms_trg[torch.tensor(list(set(in_dict['gt_correspondence'][0][:,1])))] = True
                    ...
                    shape_matching_scores = shape_matching_scores[0][gtms_src][:, gtms_trg]
                    occ_matching_scores = occ_matching_scores[0][gtms_src][:, gtms_trg]
                else:
                    shape_matching_scores = shape_matching_scores.squeeze(0)
                    occ_matching_scores = occ_matching_scores.squeeze(0)
                ...
                if self.match_selection == 'topk':
                    ...
                    if self.topk == 0:
                        ...
                        # K = int(M * N / 4) # M + N
                        K = 3 * (M + N)
                    initial_matches = topk_matching(matching_scores_before_Sinkhorn, k=K)
                elif self.match_selection == 'unidirectional_nn':
                    initial_matches = unidirectional_nn_matching(matching_scores_before_Sinkhorn, topk=self.topk)
                ...
                elif self.match_selection == 'mutual':
                    initial_matches = mutual_topk_matching(matching_scores_before_Sinkhorn, topk=self.topk)
                elif self.match_selection == 'soft':
                    initial_matches = soft_topk_matching(matching_scores_before_Sinkhorn, topk=self.topk)
                ...
                if self.auto_score_threshold or self.weighted_voting:
                    ...
                    if self.score_comb == 'sum':
                        ...
                        shape_mask = shape_matching_scores[src_idx[:], trg_idx[:]] >= score_threshold
                        occ_mask = occ_matching_scores[src_idx[:], trg_idx[:]] >= score_threshold
                        score_mask = shape_mask & occ_mask
                    src_idx, trg_idx = src_idx[score_mask], trg_idx[score_mask]


                # if self.gt_mating_surface and in_dict['gt_mating_surface'][0].sum() > 3 and in_dict['gt_mating_surface'][1].sum() > 3:
                #     src_corr_pts = src_pcd[in_dict['gt_mating_surface'][0]][src_idx]
                #     trg_corr_pts = trg_pcd[in_dict['gt_mating_surface'][1]][trg_idx]
                #     src_pcd_for_ransac = src_pcd[in_dict['gt_mating_surface'][0]]
                #     trg_pcd_for_ransac = trg_pcd[in_dict['gt_mating_surface'][1]]
                #     src_gt_normals = in_dict['gt_normals'][0][in_dict['gt_mating_surface'][0]].squeeze(0)
                #     trg_gt_normals = in_dict['gt_normals'][1][in_dict['gt_mating_surface'][1]].squeeze(0)
                
                if self.gt_mating_surface and torch.unique(in_dict['gt_correspondence'][0][:,0]).numel() > 3 and torch.unique(in_dict['gt_correspondence'][0][:,1]).numel() > 3:
                    src_corr_pts = src_pcd[gtms_src.unsqueeze(0)][src_idx]
                    trg_corr_pts = trg_pcd[gtms_trg.unsqueeze(0)][trg_idx]
                    src_pcd_for_ransac = src_pcd[gtms_src.unsqueeze(0)]
                    trg_pcd_for_ransac = trg_pcd[gtms_trg.unsqueeze(0)]
                    src_gt_normals = in_dict['gt_normals'][0][gtms_src.unsqueeze(0)].squeeze(0)
                    trg_gt_normals = in_dict['gt_normals'][1][gtms_trg.unsqueeze(0)].squeeze(0)

                else:
                    src_corr_pts = src_pcd[:,src_idx].squeeze(0) # (C, 3)
                    trg_corr_pts = trg_pcd[:,trg_idx].squeeze(0) # (C, 3)
                    src_gt_normals = in_dict['gt_normals'][0].squeeze(0)
                    trg_gt_normals = in_dict['gt_normals'][1].squeeze(0)
                    src_pcd_for_ransac = src_pcd.squeeze(0)
                    trg_pcd_for_ransac = trg_pcd.squeeze(0)
                ...
                print(f'src_corr_pts: {torch.unique(src_corr_pts, dim=0).size(0)} | trg_corr_pts: {torch.unique(trg_corr_pts, dim=0).size(0)}')
                print(f'src_pcd_for_ransac: {src_pcd_for_ransac.shape[0]} | trg_pcd_for_ransac: {trg_pcd_for_ransac.shape[0]}')
                if torch.unique(src_corr_pts, dim=0).size(0) > 3 and src_pcd_for_ransac.shape[0] > 3:
                    out_dict['less_than_3'] = 0
                    num_iters = max(math.ceil(src_corr_pts.shape[0] * 2 / 3 / 10) * 10, 100)
                    # print(f"num_iters: {num_iters}")
                    inl_R, inl_t, inliers, normals = ransac_rigid(src_corr_pts, trg_corr_pts, 
                                                src_pcd_for_ransac.squeeze(0), trg_pcd_for_ransac.squeeze(0), 
                                                next(iter(in_dict['relative_trsfm'].values())),
                                                src_gt_normals, trg_gt_normals,
                                                ...
                                                matching_choice=self.optimal_matching_choice,
                                                score_comb = self.score_comb)
                    
                    ## optimal estimation (re-estimation)
                    optimal_inliers, optimal_R, optimal_t = None, None, None
                    iteration = 0
                    # while optimal_inliers is None or not torch.equal(inliers, optimal_inliers):
                    #     iteration += 1
                    #     if iteration > 500:
                    #         break
                    #     if optimal_inliers is not None:
                    #         inliers = optimal_inliers
                    #     if optimal_R is not None and optimal_t is not None:
                    #         inl_R, inl_t = optimal_R, optimal_t
                    #     # many-to-one case : 복제하여 one-to-one으로
                    #     if self.optimal_matching_choice == 'many-to-many':
                    #         optimal_correspondences = torch.nonzero(inliers, as_tuple=False)
                    #     elif self.optimal_matching_choice == 'many-to-one':
                    #         trg_transformed = (inl_R @ trg_pcd_for_ransac.squeeze(0).T).T + inl_t
                    #         dist_mat = torch.from_numpy(cdist(src_pcd_for_ransac.squeeze(0).cpu().numpy(), trg_transformed.cpu().numpy())).to(dtype=src_pcd_for_ransac.dtype, device='cuda')
                    #         candidates = torch.stack([torch.nonzero(inliers, as_tuple=False)[:,0], torch.nonzero(inliers, as_tuple=False)[:,1], dist_mat[inliers]], dim=1)
                    #         final_selected = []
                    ...
                    #         # optimal_correspondences = final_selected[:, :2]
                    #     elif self.optimal_matching_choice == 'one-to-one':
                    #         trg_transformed = (inl_R @ trg_pcd_for_ransac.squeeze(0).T).T + inl_t
                    #         dist_mat = torch.from_numpy(cdist(src_pcd_for_ransac.squeeze(0).cpu().numpy(), trg_transformed.cpu().numpy())).to(dtype=src_pcd_for_ransac.dtype, device='cuda')
                    #         candidates = torch.stack([torch.nonzero(inliers, as_tuple=False)[:,0], torch.nonzero(inliers, as_tuple=False)[:,1], dist_mat[inliers]], dim=1)
                    ...
                    #     src_opt_pcd = src_pcd_for_ransac[src_opt_corr].squeeze(0)
                    #     trg_opt_pcd = trg_pcd_for_ransac[trg_opt_corr].squeeze(0)
                    #     trg_transformed = (inl_R @ trg_pcd_for_ransac.squeeze(0).T).T + inl_t
                    #     trg_gt_normal_transformed = torch.matmul(trg_gt_normals, inl_R.to(dtype=trg_gt_normals.dtype).T)
                    #     save_normal(f"results/test-mm-corr-{iteration}-{in_dict['filepath'].replace('/','-')}.html", [src_pcd_for_ransac[src_opt_corr], trg_transformed[trg_opt_corr]], [src_gt_normals[src_opt_corr], trg_gt_normal_transformed[trg_opt_corr]])
                    #     # breakpoint()
                    #     # print(f"src_opt_pcd : {src_opt_pcd.shape} | trg_opt_pcd : {trg_opt_pcd.shape}")
                    #     if src_opt_pcd.shape[0] < 3 or src_opt_pcd.dim() == 1:
                    #         optimal_R = inl_R
                    #         optimal_t = inl_t
                    #         out_dict['less_than_3'] = 1
                    #         # breakpoint()
                    #     else:
                    #         optimal_R, optimal_t = SVD(src_opt_pcd, trg_opt_pcd)
                    #         out_dict['less_than_3'] = 0

                    #     # print(iteration)
                    #     # inlier counting 1 : distance thresholding
                    #     trg_transformed = (optimal_R @ trg_pcd_for_ransac.squeeze(0).T).T + optimal_t
                    #     optimal_dist_mat = cdist(src_pcd_for_ransac.squeeze(0).cpu().numpy(), trg_transformed.cpu().numpy())
                    #     optimal_inliers = torch.from_numpy(optimal_dist_mat < (inlier_threshold/2)).to(dtype=inliers.dtype, device='cuda')

                    #     # inlier counting 2 : score thresholding
                    #     if self.score_comb == 'sum':
                    #         optimal_score_mask = matching_scores_before_Sinkhorn >= score_threshold
                    #     elif self.score_comb == 'intersection':
                    #         optimal_shape_mask = shape_matching_scores >= score_threshold
                    #         optimal_occ_mask = occ_matching_scores >= score_threshold
                    #         optimal_score_mask = optimal_shape_mask & optimal_occ_mask
                    ...
                    #     if self.gt_normal_threshold != -1:
                    #         optimal_R_ = optimal_R.to(dtype=trg_gt_normals.dtype)
                    #         trg_gt_normal_transformed = torch.matmul(trg_gt_normals, optimal_R_.T)
                    #         opt_cos_sim = torch.matmul(src_gt_normals, trg_gt_normal_transformed.T)
                    #         optimal_normal_mask = opt_cos_sim < self.gt_normal_threshold
                    #         # breakpoint()
                    ...
                    # R, t = optimal_R, optimal_t
                    # # save_normal(f"results/test-{in_dict['filepath'].replace('/','-')}.html", [src_pcd_for_ransac, trg_transformed], [src_gt_normals, trg_gt_normal_transformed])
                    # # src_corrrr = torch.unique(src_opt_corr)
                    # # trg_corrrr = torch.unique(trg_opt_corr)
                    # # save_normal(f"results/test-corr-{in_dict['filepath'].replace('/','-')}.html", [src_pcd_for_ransac[src_corrrr], trg_transformed[trg_corrrr]], [src_gt_normals[src_corrrr], trg_gt_normal_transformed[trg_corrrr]])
                    # # breakpoint()
                    ...
                    while optimal_inliers is None or not torch.equal(inliers, optimal_inliers):
                        ...
                        trg_transformed = (inl_R @ trg_pcd_for_ransac.squeeze(0).T).T + inl_t
                        optimal_dist_mat = cdist(src_pcd_for_ransac.squeeze(0).cpu().numpy(), trg_transformed.cpu().numpy())
                        optimal_inliers = torch.from_numpy(optimal_dist_mat < 0.008).to(dtype=inliers.dtype, device='cuda')
                        ...
                        # inlier counting 3 : normal thresholding
                        if self.gt_normal_threshold != -1:
                            inl_R_ = inl_R.to(dtype=trg_gt_normals.dtype)
                            trg_gt_normal_transformed = torch.matmul(trg_gt_normals, inl_R_.T)
                            opt_cos_sim = torch.matmul(src_gt_normals, trg_gt_normal_transformed.T)
                            temp_optimal_normal_mask = opt_cos_sim < -0.9 #self.gt_normal_threshold
                            if iteration > 1 and temp_optimal_normal_mask.sum() < optimal_normal_mask.sum():
                                break
                        ...
                        # trg_transformed = (inl_R @ trg_pcd_for_ransac.squeeze(0).T).T + inl_t
                        # trg_gt_normal_transformed = torch.matmul(trg_gt_normals, inl_R.to(dtype=trg_gt_normals.dtype).T)
                        # save_normal(f"test_results/corr-{iteration}-{in_dict['filepath'].replace('/','-')}.html", [src_pcd_for_ransac[src_opt_corr], trg_transformed[trg_opt_corr]], [src_gt_normals[src_opt_corr], trg_gt_normal_transformed[trg_opt_corr]])
                        # breakpoint()
                        # print(f"src_opt_pcd : {src_opt_pcd.shape} | trg_opt_pcd : {trg_opt_pcd.shape}")
                        if src_opt_pcd.shape[0] < 3 or src_opt_pcd.dim() == 1:
                            optimal_R = inl_R
                    ...
                    R, t = optimal_R, optimal_t
                    # save_normal(f"test_results/test-final-{in_dict['filepath'].replace('/','-')}.html", [src_pcd_for_ransac, trg_transformed], [src_gt_normals, trg_gt_normal_transformed])
                    # src_corrrr = torch.unique(src_opt_corr)
                    # trg_corrrr = torch.unique(trg_opt_corr)
                    # save_normal(f"test_results/test-final-corr-{in_dict['filepath'].replace('/','-')}.html", [src_pcd_for_ransac[src_corrrr], trg_transformed[trg_corrrr]], [src_gt_normals[src_corrrr], trg_gt_normal_transformed[trg_corrrr]])
                    # breakpoint()

                else:
                    R, t = SVD(src_corr_pts, trg_corr_pts)
                    trg_transformed_temp = (R @ trg_pcd_for_ransac.squeeze(0).T).T + t
                    inliers = torch.from_numpy(cdist(src_pcd_for_ransac.squeeze(0).cpu().numpy(), trg_transformed_temp.cpu().numpy()) < inlier_threshold).to(dtype=src_pcd_for_ransac.dtype, device='cuda')
                    out_dict['less_than_3'] = 1
                    # breakpoint()
                    print("Why am i here?")
                ...
                inliers_upperbound = torch.from_numpy(dist_whole_match < inlier_threshold).to(dtype=inliers.dtype, device='cuda')
                inliers_count_upper = torch.count_nonzero(torch.sum(inliers_upperbound, dim=0))
                min_mask = torch.from_numpy(dist_whole_match == np.min(dist_whole_match, axis=0)).to(dtype=inliers.dtype, device='cuda')
                ...
                out_dict['estimated_rotat'] = R.T
                out_dict['estimated_trans'] = t @ R
                # -(estimated_transform[:3, :3].inverse() @ -estimated_transform[:3, 3]) ## 둘이 비슷한거 같은데 왜지?
