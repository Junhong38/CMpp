...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        b, c, n = x.size()
        y = self.global_pooling(x)  # Apply global pooling
        y = y.view(b, c)  # Flatten the pooling result
        y = self.fc1(y)  # First fully connected layer
        y = self.relu(y)  # ReLU activation
        y = self.fc2(y)  # Second fully connected layer
        y = self.sigmoid(y).view(b, c, 1)  # Sigmoid activation and reshape
        return x * y.expand_as(x)

class EquiAssem_occ_v6(pl.LightningModule):
    def __init__(self, lr, backbone='eqcnn', visualize=False):
        super(EquiAssem_occ_v6, self).__init__()
        ...
        self.global_pooling_shape = SEBlock(channels=1023)
        self.global_pooling_occ = SEBlock(channels=1023)
    ...
    def forward_pass(self, in_dict, mode, optimizer_idx):
        ...
        src_shape_feats = self.global_pooling_shape(src_shape_feats)
        trg_shape_feats = self.global_pooling_shape(trg_shape_feats)
        src_occ_feats = self.global_pooling_occ(src_occ_feats)
        trg_occ_feats = self.global_pooling_occ(trg_occ_feats)
        ...
        matching_scores = torch.einsum('b c n , b c m -> b n m', src_inv_feats_before , trg_inv_feats_before) # (1, N, M)
