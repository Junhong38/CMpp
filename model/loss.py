...
class CircleLossOcc(nn.Module):

    def __init__(self, log_scale=16, pos_optimal=0.1, neg_optimal=1.4):
        super(CircleLossOcc,self).__init__()
        self.log_scale = 24
        self.pos_optimal = pos_optimal
        self.neg_optimal = neg_optimal
    ...
    def get_circle_loss(self, coords_dist, feats_dist):
        ...
        # pos_mask = coords_dist < self.pos_radius
        # neg_mask = coords_dist > self.safe_radius
        neg_mask = coords_dist < self.pos_radius
        pos_mask = coords_dist > self.safe_radius
    ...
    def get_recall(self, coords_dist, feats_dist):
        ...
        n_gt_pos = (pos_mask.sum(-1)>0).float().sum()+1e-12
        try:
            _, sel_idx = torch.max(feats_dist, -1)
        except:
            return torch.tensor(0.).to(feats_dist.device)
        sel_dist = torch.gather(coords_dist,dim=-1,index=sel_idx[:,None])[pos_mask.sum(-1)>0]
        n_pred_pos = (sel_dist < self.pos_radius).float().sum()
    ...
    def forward(self, src_occ, trg_occ, correspondence):
        ...
        with torch.no_grad():
            if src_non_corr_indices and trg_non_corr_indices:
                ...
                loss_non_corr = loss_non_corr / src_num_points  # 평균을 내기 위해 나눠줌
...
class OrientationLossBoth(nn.Module):
    ...
    def intra_loss(self, in_ori, ex_ori):
        diff = in_ori - ex_ori.transpose(2, 3)
        f_norm = torch.norm(diff, p='fro', dim=(2, 3))
        intra_loss = torch.mean(f_norm ** 2)
        return intra_loss
    ...
    def forward(self, src_in_ori, trg_in_ori, src_ex_ori, trg_ex_ori, correspondence, gt_rot):
        ...
        src_gt_rot = gt_rot[0]
        trg_gt_rot = gt_rot[1]
        intra_loss = (self.intra_loss(src_in_ori, src_ex_ori) + self.intra_loss(trg_in_ori, trg_ex_ori))/2
        ...
        ori_loss = intra_loss + inter_loss
...
class OrientationLossBothGeodesic(nn.Module):
    ...
    def intra_loss(self, in_ori, ex_ori):
        R_diff = torch.matmul(in_ori, ex_ori.transpose(2,3))
        trace_R_diff = torch.einsum('bnii->bn', R_diff)
        theta = torch.acos(torch.clamp((trace_R_diff-1)/2, -1.0+self.eps, 1.0-self.eps))
        intra_loss = torch.mean((theta - torch.pi) ** 2)
        return intra_loss
