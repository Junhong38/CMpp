...
class CircleLoss(nn.Module):
    ...
    def get_circle_loss(self, coords_dist, feats_dist):
        ...
        return circle_loss, P_margin.mean().detach().cpu(), None #N_margin.mean().detach().cpu()

######
    # def get_circle_loss(self, coords_dist, feats_dist):
    #     """
    #     Modified from: https://github.com/TinyZeaMays/CircleLoss/
    #     """
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, src_pcd, tgt_pcd, src_feats, tgt_feats, correspondence):
        ...
        src_feats = F.normalize(src_feats.squeeze(0), p=2, dim=-1)
        tgt_feats = F.normalize(tgt_feats.squeeze(0), p=2, dim=-1)
        feats_dist = torch.sqrt(torch.clamp(2.0 - 2.0 * torch.einsum('x d, y d -> x y', src_feats, tgt_feats)).pow(0.5))
