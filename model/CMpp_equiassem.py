...
class EquiAssem(pl.LightningModule):
    ...
    def _calculate_recall(self, matching_scores_drop, gt_corr, topks=[1,2,3]): #topk_ratios=[0.1, 0.2, 0.4, 0.8, 1., 2.]):
        """
        ...
        for topk in topks:
            ...
            # recall_src = (topk_mask_src * correspondence_mask).sum(dim=-1)[correspondence_mask_src] / correspondence_mask.sum(dim=-1)[correspondence_mask_src]
            # precision_src = (topk_mask_src * correspondence_mask).sum(dim=-1)[correspondence_mask_src] / topk_mask_src.sum(dim=-1)[correspondence_mask_src]
            ...
            # recall_trg = (topk_mask_trg * correspondence_mask).sum(dim=-2)[correspondence_mask_trg] / correspondence_mask.sum(dim=-2)[correspondence_mask_trg]
            # precision_trg = (topk_mask_trg * correspondence_mask).sum(dim=-2)[correspondence_mask_trg] / topk_mask_trg.sum(dim=-2)[correspondence_mask_trg]
