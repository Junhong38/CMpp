...
def _transform(pcd, rotat=None, trans=None, rotate_first=True):
    ...
    if rotate_first:
        ...
        return torch.einsum('x y, n y -> n x', rotat, pcd + trans) # _multi_part_assemble
...
def test(args):
    ...
    for idx, in_dict in enumerate(dataloader_test):
        ...
        ##### self-checking by GT #####
        # in_dict['pcd_t'] = in_dict['pcd']
        # normal = []
        # for i, pcdt in enumerate(in_dict['pcd']):
        #     normal.append(_transform(pcdt.squeeze(0), trans=-in_dict['gt_trans'][i].cpu()).unsqueeze(0))
        # in_dict['pcd_t'] = normal
        ...
        for i in range(in_dict['n_frac'].item()):
            ...
            ### self-checking ###
            # relative_rotat = Rotation.from_matrix(in_dict['relative_trsfm'][f'{i}-{max_idx}'][0].cpu().numpy()).as_quat().squeeze(0)
            # relative_trans = in_dict['relative_trsfm'][f'{i}-{max_idx}'][1].cpu().numpy().squeeze(0)
