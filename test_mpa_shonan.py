...
def save_ransaced_mesh(in_dict, out_dict, crd, cd, pa_crd, pa_cd, folder_name):
    ...
    for rotat, trans in zip(out_dict['aligned_pred_rotat'], out_dict['aligned_pred_trans']):
    # for rotat, trans in zip(in_dict['aligned_gt_rotat'], in_dict['aligned_gt_trans']): # for gt save
        mesh[idx].vertices = (rotat.cpu() @ (torch.tensor(mesh[idx].vertices).float() + trans.cpu()).T).T
        idx+=1
...
def test(args):
    ...
    for idx, in_dict in enumerate(dataloader_test):
        ...
        for pair_idx in pair_indices:
            ...
            #     in_dict['pcd_t'][pair_idx0] = in_dict['pcd_t'][pair_idx0][in_dict['gt_mating_surface'][f'{pair_idx0}-{pair_idx1}'][0]].unsqueeze(0)
            #     in_dict['pcd_t'][pair_idx1] = in_dict['pcd_t'][pair_idx1][in_dict['gt_mating_surface'][f'{pair_idx0}-{pair_idx1}'][1]].unsqueeze(0)
            #     in_dict['pcd'][pair_idx0] = in_dict['pcd'][pair_idx0][in_dict['gt_mating_surface'][f'{pair_idx0}-{pair_idx1}'][0]].unsqueeze(0)
            #     in_dict['pcd'][pair_idx1] = in_dict['pcd'][pair_idx1][in_dict['gt_mating_surface'][f'{pair_idx0}-{pair_idx1}'][1]].unsqueeze(0)
            #     breakpoint()
            if (args.html_vis and args.initial_match_selection == 'topk' and in_dict['anchor_idx'] == pair_idx0) or (not args.html_vis) or (args.html_vis and args.initial_match_selection == 'unidirectional_nn' and (in_dict['pcd'][pair_idx0].size(1) >= in_dict['pcd'][pair_idx1].size(1))):
                in_dict_pair = {
        ...
        if not args.html_vis:
            if args.registration == 'wsvd':
                ...
                for i in range(0, in_dict['n_frac']):
                    if i == anchor_idx: 
                        ...
                        if f'{anchor_idx}-{i}' in out_dict.keys():
                            ...
                            aligned_pred_rotat.append(out_dict[f'{i}-{anchor_idx}']['estimated_rotat'].squeeze(0))
                            aligned_pred_trans.append(out_dict[f'{i}-{anchor_idx}']['estimated_trans'])
                ...
                for i in range(0, in_dict['n_frac']):
                    if i == anchor_idx:
                        ...
                        if f'{anchor_idx}-{i}' in out_dict.keys():
                            aligned_gt_rotat.append(in_dict['relative_trsfm'][f'{anchor_idx}-{i}'][0].squeeze(0))
                            aligned_gt_trans.append(in_dict['relative_trsfm'][f'{anchor_idx}-{i}'][1].squeeze(0))
                            best_CD = out_dict[f'{anchor_idx}-{i}']['best_CD']
                            min_CD = out_dict[f'{anchor_idx}-{i}']['min_CD']
                        else:
                            aligned_gt_rotat.append(in_dict['relative_trsfm'][f'{i}-{anchor_idx}'][0].squeeze(0))
                            aligned_gt_trans.append(in_dict['relative_trsfm'][f'{i}-{anchor_idx}'][1].squeeze(0))
                            best_CD = out_dict[f'{i}-{anchor_idx}']['best_CD']
                            min_CD = out_dict[f'{i}-{anchor_idx}']['min_CD']
                ...
                result_str = f'{idx}/{total} | #-Part: {len(pcds_pred)} | CRD: {round(crd,2)} | CD: {round(cd,2)} | RRMSE: {round(rrmse,2)} | TRMSE: {round(trmse,2)} | PA(cd): {round(pa,2)} | PA(crd): {round(pa_crd,2)}'
                print(result_str)
                print(f"best CD: {best_CD} | min_CD: {min_CD}")
                if pa < 1:
                    # breakpoint()
                    save_pc('./test_score_nonormal_mating/test_'+in_dict['filepath'].replace('/','_')+'.pcd', pcds_pred)
                # with open(args.results_folder, "a", encoding="utf-8") as f:
                #     f.write(result_str + "\n")
                #     f.write(f"best CD: {best_CD} | min_CD: {min_CD} \n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--initial_match_selection', type=str, default='unidirectional_nn', choices=['topk', 'unidirectional_nn', 'injective', 'bijective', 'reciprocal'])
    ...
    parser.add_argument('--vis_folder', type=str, default='ransac')
